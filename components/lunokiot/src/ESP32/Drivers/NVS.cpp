...
static char current_namespace[16] = "nvs";
...
static esp_err_t store_blob(nvs_handle_t nvs, const char *key, const char *str_values)
{
    ...
    if (str_len % 2) {
        printf("Blob data must contain even number of characters\n");
    ...
    for (int i = 0, j = 0; i < str_len; i++) {
        ...
        if (ch >= '0' && ch <= '9') {
            ...
            printf("Blob data contain invalid character\n");
            free(blob);
            return ESP_ERR_NVS_TYPE_MISMATCH;
        }
...
static esp_err_t set_value_in_nvs(const char *key, const char *str_type, const char *str_value)
{
    ...
    if (err == ESP_OK) {
        err = nvs_commit(nvs);
        if (err == ESP_OK) {
            printf("Value stored under key '%s'\n", key);
        }
    }
...
static esp_err_t erase(const char *key)
{
    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK) {
            err = nvs_commit(nvs);
            if (err == ESP_OK) {
                printf("Value with key '%s' erased\n", key);
            }
        }
        nvs_close(nvs);
    }
...
static int erase_namespace(int argc, char **argv)
{
    ...
    const char *name = erase_all_args.nvs_namespace->sval[0];
...
static int set_namespace(int argc, char **argv)
{
    ...
    const char *nvs_namespace = namespace_args.nvs_namespace->sval[0];
    strlcpy(current_namespace, nvs_namespace, sizeof(current_namespace));
    printf("Namespace set to '%s'\n", current_namespace);
    return 0;
}
...
static int list_entries(int argc, char **argv)
{
    ...
    list_args.nvs_namespace->sval[0] = "";
    ...
    const char *name = list_args.nvs_namespace->sval[0];
...
void register_nvs(void) {
    ...
    erase_all_args.nvs_namespace = arg_str1(NULL, NULL, "<namespace>", "namespace to be erased");
    erase_all_args.end = arg_end(2);

    namespace_args.nvs_namespace = arg_str1(NULL, NULL, "<namespace>", "namespace of the partition to be selected");
    namespace_args.end = arg_end(2);

    list_args.partition = arg_str1(NULL, NULL, "<partition>", "partition name");
    list_args.nvs_namespace = arg_str0("n", "namespace", "<namespace>", "namespace name");
